...
//!
//! The discovery pass evaluates every raw program exactly once per discovery
//! history. The resulting output vector is retained with the executable program,
//! so transport scoring never re-executes a program on an original history.
//! This keeps the preregistered 13,248 discovery evaluations and 2,304
//! transformed-history evaluations exact.
...
#[derive(Debug, Clone)]
struct EvaluatedRepresentative {
    program: RefinementProgram,
    discovery_outputs: Vec<bool>,
}
...
fn search(
    ...
    let mut partitions = BTreeMap::<Vec<bool>, Vec<EvaluatedRepresentative>>::new();
    for program in programs {
        let mut discovery_outputs = Vec::with_capacity(problem.discovery.len());
        for episode in &problem.discovery {
            budget.discovery_program_history_evaluations = budget
                .discovery_program_history_evaluations
                .saturating_add(1);
            discovery_outputs.push(program.execute(&episode.history));
        }
        let canonical = canonical_partition(discovery_outputs.clone());
        partitions
            .entry(canonical)
            .or_default()
            .push(EvaluatedRepresentative {
                program,
                discovery_outputs,
            });
    }
    ...
    let mut ranked_partitions = partitions
        ...
        .map(|(partition, mut representatives)| {
            representatives.sort_by(|left, right| {
                left.program
                    .canonical_string()
                    .cmp(&right.program.canonical_string())
            });
            ...
            let canonical = representatives
                .first()
                .map(|representative| representative.program.canonical_string())
                .unwrap_or_default();
    ...
    for representative in &winner.4 {
        ...
        for transformation in &suite.transformations {
            for (history_index, episode) in problem.discovery.iter().enumerate() {
                ...
                budget.transport_program_history_evaluations = budget
                    ...
                    .saturating_add(1);
                let transformed_output = representative.program.execute(&transformed);
                ...
                if transformed_output != representative.discovery_outputs[reference_index] {
        ...
        transport_rank.push((
            violations,
            representative.program.canonical_string(),
            representative.program.clone(),
        ));
    }

    transport_rank.sort_by(|left, right| {
        left.0
            .cmp(&right.0)
...
mod tests {
    ...
    #[test]
    fn moving_orbit_selects_exact_stable_frontier_without_hidden_reexecution() {
    ...
    fn rewired_correspondence_has_no_zero_violation_certificate() {
        ...
        assert_eq!(validation_budget.discovery_program_history_evaluations, 13_248);
