...
fn wrong_suite() -> TransformationSuite {
    TransformationSuite {
        transformations: vec![
            BlockPermutation {
                block_width: 2,
                block_order: vec![0, 3, 1, 2],
            },
            BlockPermutation {
                block_width: 2,
                block_order: vec![0, 1, 3, 2],
            },
        ],
        correspondence: CorrespondenceMode::SameHistory,
    }
...
fn heldout_transformations() -> [BlockPermutation; HELDOUT_TRANSFORMATIONS] {
    [
        BlockPermutation {
            block_width: 2,
            block_order: vec![1, 3, 2, 0],
        },
        BlockPermutation {
            block_width: 2,
            block_order: vec![3, 1, 0, 2],
        },
    ]
}
...
fn build_roots() -> Result<Vec<RootTask>, Box<dyn Error>> {
    ...
    for (family_index, family) in FAMILIES.iter().enumerate() {
        for root_index in 0..ROOTS_PER_FAMILY {
            ...
            let correct_intervention =
                Atom::new(format!("{family}_{root_index}_correct_ancestor_probe"))?;
            let wrong_intervention =
                Atom::new(format!("{family}_{root_index}_wrong_ancestor_probe"))?;
            let stage2_intervention = Atom::new(format!("{family}_{root_index}_stage2_probe"))?;
            let p0 = Atom::new(format!("{family}_{root_index}_p0"))?;
            let p1 = Atom::new(format!("{family}_{root_index}_p1"))?;
            let s0 = Atom::new(format!("{family}_{root_index}_s0"))?;
            let s1 = Atom::new(format!("{family}_{root_index}_s1"))?;
            let z0 = Atom::new(format!("{family}_{root_index}_z0"))?;
            let z1 = Atom::new(format!("{family}_{root_index}_z1"))?;
            ...
            for p in [false, true] {
                for q in [false, true] {
                    for r in [false, true] {
                        for s in [false, true] {
                            let history = pair_block_history(
                            ...
                            correct_discovery.push(WitnessedHistory {
                                ...
                                intervention: correct_intervention.clone(),
                                outcome: if p { p1.clone() } else { p0.clone() },
                            });
                            wrong_discovery.push(WitnessedHistory {
                                ...
                                outcome: if s { s1.clone() } else { s0.clone() },
                            });
                            stage2_discovery.push(WitnessedHistory {
                                ...
                                outcome: if p != r { z1.clone() } else { z0.clone() },
                            });
...
fn predict_transfer(
    root: &RootTask,
    language: &TransportDescendantStateLanguage,
...
fn path_metrics<'a>(report: &'a SplitReport, path: PathKind) -> &'a PathMetrics {
    report
        .paths
        .get(path.name())
        .unwrap_or_else(|| panic!("missing ΩD1 path metrics for {}", path.name()))
}
