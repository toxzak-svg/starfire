...
pub enum RetrievalError {
    ...
    #[error(
        "retrieval byte limit {requested} is outside the supported range; hard max is {hard_max}"
    )]
        ...
        loop {
            ...
            if is_redirect(status) {
                ...
                let location = response.header("Location").ok_or_else(|| {
                    RetrievalError::RedirectMissingLocation {
                        url: current.as_str().to_string(),
                    }
                })?;
...
impl ParsedHttpUrl {
    ...
    fn resolve(&self, location: &str, policy: &RetrievalPolicy) -> Result<Self, RetrievalError> {
...
pub fn classify_content(content_type: Option<&str>, bytes: &[u8]) -> RetrievedKind {
    ...
    match mime.as_str() {
        ...
        "application/json" | "application/ld+json" | "application/geo+json" => RetrievedKind::Json,
...
mod tests {
    ...
    fn relative_redirect_is_normalized() {
        ...
        let base =
            ParsedHttpUrl::parse("http://example.com/a/b/start", &policy).expect("base URL parses");
        let resolved = base
