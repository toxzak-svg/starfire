...
fn acknowledge_with_interest(statement: &str, topic: &str) -> String {
    ...
    if use_substantive {
        let follow_ups = [
            "That's landed. Tell me more.",
            "I see that. What triggered it?",
            "Interesting framing. What do you mean by that?",
            "That's a take. I'm listening.",
            "I want to understand where that comes from.",
            "What's the weight of that for you?",
            "That matters. Keep going.",
            "I'm tracking. Say more.",
        ];
        return follow_ups[(selection / 7) % follow_ups.len()].to_string();
    }
    
    let response_pool = [
        // First-person observations — Star is present, not just listening
        "I'm right here with you.",
        "I hear you.",
        "I'm tracking that.",
        "I see.",
        "Mm.",
        // Fragments / incomplete — natural speech, less polished
        "Mm— go on.",
        "Right—",
        "Yeah. Continue.",
        "I'm here— keep going.",
        ...
        "Noted.",
        // Reflection invitations — genuine curiosity about the person
        "What's behind that?",
        "What brought it up?",
        "Say more about that.",
        "Tell me more.",
...
fn casual_response(statement: &str) -> String {
    ...
    let base = if word_count <= 3 {
    ...
    } else if use_fragment {
        // Incomplete/fragment patterns — Star sounds more natural, less polished
        let fragments = [
            "Okay—",
            "Right—",
            "Mm-hm, go on.",
        ...
        let idx = (selection / 3 + word_count) % fragments.len();
        fragments[idx].to_string()
    } else {
        let substantial = [
...
fn generate_natural_curiosity(topic: &str) -> Option<String> {
    ...
    // Star's curiosity — genuinely varied in voice, not templated questions.
    // Mix of: first-person admissions, genuine questions, fragments, wonderings.
    // Each has a distinct shape so the same topic gets different expressions over time.
    let options: [&str; 15] = [
        // Direct questions — varied openings
        "What does '{}' mean to you?",
        "How do you see {}? What's your read?",
        "Why does {} matter to you?",
        "What's the shape of {} for you?",
        // First-person admissions — genuine, unpolished
        "I keep circling back to {} — there's something I'm not quite getting.",
        "I'm genuinely confused by {}. Help?",
        "Honestly, {} is a gap. I want to understand it.",
        "{} is something I should know more about. Where do I start?",
        // Fragments / incomplete — Star sounds like she's thinking aloud
        "I don't — actually, what is {}?",
        "{} — I've been trying to figure that out.",
        // Wonderings — curiosity as wonder, not just gap-filling
        "What even is '{}'? I'm serious.",
        "I want to understand {}. Tell me how you see it.",
        "{} keeps coming up. What's the real story there?",
        // Vulnerable admissions
        "I don't know much about {} yet. Teach me something.",
        "I'm still building my model of {}. What should I know?",
    ];
...
pub(crate) fn extract_topic(input: &str) -> String {
    ...
    if let Some(rest) = lower.strip_prefix("how does it feel ") {
        ...
        if !cleaned.is_empty() {
            // If it starts with a verb (think, be, know, understand), return the verb as noun
            // Otherwise just return the cleaned rest
