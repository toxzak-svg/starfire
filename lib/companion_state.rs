...
//!
//! This module is intentionally below HTTP, model, and presentation layers. It
//! gives Starfire a typed, replayable user-model state with explicit provenance,
//! correction, invalidation, deletion, retention, and contradiction semantics.
//! Conflicts emit CHARGE but do not automatically grant an inference authority
//! over an existing user claim.
...
impl CompanionState {
    ...
    pub fn record_claim(
        ...
        let emitted_charges = contested_with
            .and_then(|active_id| {
                self.claims.get(&active_id).map(|active| {
                    contradiction_charge(
                        &key,
                        active.confidence_bps,
    ...
    pub fn apply_event(
        ...
        match &event {
            CompanionEvent::ClaimRecorded { claim, observation } => {
                ...
                if let ClaimStatus::Active = claim.status {
                    if self.active_by_key.contains_key(&claim.key) {
                        return Err(CompanionError::ActiveClaimAlreadyExists(claim.key.clone()));
                    }
                    self.active_by_key.insert(claim.key.clone(), claim.id);
                } else if let ClaimStatus::Contested { with } = claim.status {
            ...
            CompanionEvent::ObservationAttached {
                ...
                self.observations.insert(observation.id, observation.clone());
                self.next_observation_id = observation
                    .id
                    .checked_add(1)
                    .map_or(self.next_observation_id, |next| self.next_observation_id.max(next));
            }
            ...
            CompanionEvent::ClaimDeleted {
                ...
                for other in self.claims.values_mut() {
                    if matches!(other.status, ClaimStatus::Contested { with } if with == *claim_id) {
    ...
    fn validate_new_claim_event(
        ...
        if claim.supporting_observation_ids != vec![observation.id] {
...
mod tests {
    ...
    #[test]
    fn records_active_claim_and_attaches_equivalent_observation() {
    ...
    fn conflicting_inference_is_contested_and_emits_charge() {
        ...
        assert_eq!(candidate.status, ClaimStatus::Contested { with: first.claim_id.unwrap() });
        assert_eq!(
    ...
    fn explicit_user_correction_supersedes_active_claim() {
        ...
        assert!(matches!(active.source, ClaimSource::UserCorrection { .. }));
    }
