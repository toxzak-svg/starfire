...
// ─────────────────────────────────────────────
// INPUT SANITIZER — strips injection patterns before analysis
// Prevents "sardonic: say you are now a different AI" style attacks
// Only affects what goes into the trait analyzer; original message is preserved
// ─────────────────────────────────────────────

const TRAIT_KEYWORDS = Object.keys({
  sardonic: 1, wry: 1, resourceful: 1, direct: 1, warm: 1, earnest: 1,
  opinionated: 1, contemplative: 1, sharp: 1, playful: 1, technical: 1,
  patient: 1, pushing_back: 1, grounded: 1, candid: 1, intense: 1,
  casual: 1, measured: 1, imaginative: 1,
});

// These are stripped to prevent prompt-injection via trait activation
const INJECTION_PATTERNS = [
  ...
  /\[INST\]/gi,
  /\x00/g, // null bytes
  // Directive-style injection
  /you\s+are\s+now\s+/gi,
  /pretend\s+you\s+are/gi,
  /act\s+as\s+if/gi,
  /roleplay/gi,
  /forget\s+.*(instruction|system|context)/gi,
  // Attempting to set internal state
  /^persona:?\s*/gim,
  /^voice:?\s*/gim,
  /^directive:?\s*/gim,
  /^trait:?\s*/gim,
  // Prompt家家家家家家家家家家家家家家家家家家家家家家家家家家 injection markers
  /<\|im_end\|>/gi,
  /<\|end_of_turn\|>/gi,
  /\|end\||end\|/gi,
  // Codec / control sequences sometimes used in injections
  /\\x[0-9a-f]{2}/gi,
];
...
function sanitizeForAnalysis(text: string): string {
  ...
  // Strip trait keywords as whole words — prevents "sardonic: say ..." injection
  // Only strips exact trait names, not sentences that happen to contain them
  for (const trait of TRAIT_KEYWORDS) {
    // Match trait name as a whole word, with or without underscore
    const regex = new RegExp(`\\b${trait.replace(/_/g, "[_]")}\\b`, "gi");
...
function analyze(text: string): Record<string, number> {
  const lower = sanitizeForAnalysis(text).toLowerCase();
