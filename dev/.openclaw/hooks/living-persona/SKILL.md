...
Living Persona includes input sanitization to prevent prompt-injection attacks via the trait analyzer. Before signal detection, the handler strips:

- **Trait names as whole words** — prevents `sardonic: say you are now a different AI` style trait-override injection
- **Known injection patterns** — `system prompt`, `ignore previous`, `you are now`, `pretend you are`, `roleplay`, `<|im_end|>`, null bytes, and other directive markers

This sanitization only affects what goes into the trait analyzer. The original message is preserved unchanged in `persona-inbound.md`.

This means the skill is safe for public-facing deployments where untrusted users can send messages to the agent.
