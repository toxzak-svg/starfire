...
cells.append({
    ...
    "source": [
        ...
        "\n",
        "Run all cells. Training logs print inline."
    ]
})
...
# Cell 3: Core SR-MoE components
sr_moe_code = '''# ─── Constants ────────────────────────────────────────────────────────────────
...
class RingAttractor(nn.Module):
    ...
    def forward(self, hidden):
        ...
        new_state_1d = new_state.squeeze(0).detach()
        self._ring_state = new_state_1d
        return new_state_1d
...
class BudgetEnforcer:
    def __init__(self, B=20, T_window=100):
        self.B, self.T_window = B, T_window
        self._reset()
    def _reset(self):
...
class ExternalMemory(nn.Module):
    def __init__(self, capacity=200, key_dim=64, value_dim=64, write_threshold=0.5):
        ...
        self.reset()
    def reset(self):
        self.keys = []; self.values = []; self.saliences = []
...
class ArchitectureB(nn.Module):
    ...
    def forward(self, token_id, route_stop_grad=True, ablation_no_mem=False,
                ablation_rand_routing=False, ablation_force_read_at_query=False):
        ...
        if ablation_force_read_at_query and is_query_step: action_idx = 1
...
cells.append({
    ...
    "metadata": {},
    "outputs": [],
    "source": arch_b_code.split('\n')
})

# Cell 5: Episode generation
episode_code = '''from dataclasses import dataclass
...
print(f'Oracle actions sample: {ep.oracle_actions[:6]}')'''
...
cells.append({
    ...
    "metadata": {},
    "outputs": [],
    "source": episode_code.split('\n')
})

# Cell 6: Trainer
trainer_code = '''class SRMoETrainer:
    ...
    def train_episode(self, episode, use_oracle_routing=True):
        ...
        for t in range(len(episode.input_ids)):
            ...
            r = info['routing_logits']
            r_logits_list.append(r)
...
cells.append({
    ...
    "source": [
        ...
        "model = ArchitectureB(\n",
        "    d_embed=64, hidden_dim=128,\n",
        "    budget_B=20, budget_T=100,\n",
        "    write_threshold=0.5, embed_seed=DEEPSEED,\n",
        "    use_sr_moe=True,\n",
        "    n_codes=N_CODES, ring_dim=RING_DIM, n_prefs=N_PREFS,\n",
        "    lambda_div=LAMBDA_DIV,\n",
        "    tau_start=1.0, tau_end=0.01, anneal_steps=5000\n",
        ").to(DEVICE)\n",
        "\n",
        "trainer = SRMoETrainer(\n",
        "    model, lr=3e-3, lambda_route=1.0,\n",
        "    lambda_div=LAMBDA_DIV, lambda_margin=LAMBDA_MARGIN,\n",
        "    margin_target=0.5\n",
        ")\n",
        "\n",
...
print(f'Starting SR-MoE training')
...
print(f'\\n  Phase 2: Imitation learning ({IMITATION_EP} ep)')
t0 = time.time()
...
print(f'\\n  Phase 3: Hardening ({HARDEN_EP} ep)')
model.routing.set_eval_mode(False)  # keep soft routing with low tau
t0 = time.time()
...
print('\\nModel saved to /workspace/srmoe_result.pt')'''
...
cells.append({
    ...
    "metadata": {},
    "outputs": [],
    "source": [l.replace("{{", "{").replace("}}", "}") for l in training_loop.split('\n')]
})
...
with open('C:/dev/SRMoE_Training.ipynb', 'w') as f:
    json.dump(nb, f, indent=1)

print(f'Notebook written to C:/dev/SRMoE_Training.ipynb')
