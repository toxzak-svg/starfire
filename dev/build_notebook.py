"""Generate SRMoE_Training.ipynb as properly formatted JSON."""
import json, os

NL = "\n"

def c(*lines):
    return {"cell_type": "code", "execution_count": None, "metadata": {}, "outputs": [], "source": NL.join(lines)}
def m(*lines):
    return {"cell_type": "markdown", "metadata": {}, "source": NL.join(lines)}
...
cells = [
    m(
        ...
        "",
        "Run all cells. GPU recommended.",
        "",
        "## Architecture",
        "1. **Popcode Router** — selects discrete codeword from learned VQ codebook (not softmax)",
        "2. **Ring Attractor** — 2D continuous attractor for path-dependent routing state",
        "3. **Diversity Regularizer** — prevents code collapse via usage entropy + discrimination loss",
        "",
    ...
    c(
        ...
        "            return True",
        "        window_start = max(0, len(self.accesses) - self.T_window)",
        "        window = self.accesses[window_start:]",
    ...
    c(
        ...
        "        if i % 2 == 0:  # key token",
        "            key_idx = i // 2",
        "            if key_idx < N_GENUINE:",
        "                input_ids.append(genuine_keys[key_idx])",
        "                oracle_actions.append('PERCEIVE')",
        "            else:",
        "                input_ids.append(filler_keys[key_idx])",
        "                oracle_actions.append('SKIP')",
        "        else:  # value token",
        "            val_idx = i // 2",
        "            if val_idx < N_GENUINE:",
        "                input_ids.append(genuine_values[val_idx])",
        "                oracle_actions.append('WRITE')",
        "            else:",
        "                input_ids.append(filler_values[val_idx])",
        "                oracle_actions.append('SKIP')",
        "",
        "    # Delay: 50 steps",
        "    for _ in range(50):",
