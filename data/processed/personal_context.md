...
## About Zach
['Building things that run locally without GPU dependency', 'Creating AI that has genuine personality, not just templated responses', 'Pushing the boundary on 1-bit / extreme quantization', 'Making AI that can actually act on the world (agents, not just chat)', 'Privacy-first: data stays local, no cloud dependency']
